...
(note) =org-tree-slide-skip-comments-toggle= will switch between ~t~ and ~nil~ normally, but if org-tree-slide-skip-comments is specified as ~'inherit~, then the toggle will switch between ~'inherit~ and ~t~.
